...
// Layer 2 perception
import { getSemanticSurface } from './layer2/semanticSurface.js';
...
server.registerTool(
  'browser_launch',
  {
    description:
      ...
      'If a URL is provided, the browser navigates to it immediately after launch. ' +
  ...
  async ({ headless, userDataDir, url }) => {
    const result = await connectionManager.launch({ headless, userDataDir, url });
...
server.registerTool(
  ...
  {
    description:
      'Navigate the active browser tab to a new URL. Waits for the page load event before returning. ' +
  ...
  async ({ url }) => {
    requireSession();
    const result = await connectionManager.navigate(url);
...
server.registerTool(
  'atomic_interact',
  {
    ...
    inputSchema: {
      action: z.enum(['click', 'type', 'hover', 'key', 'scroll']).describe('The interaction action to perform'),
  ...
  async ({ action, backendNodeId, coordinate, text, key, direction, amount, timeoutMs }) => {
    ...
    switch (action) {
      ...
      case 'type':
        ...
        if (backendNodeId !== undefined) {
          result = await atomicType(page, cdp, backendNodeId, text, tel, { timeoutMs });
        } else if (coordinate) {
...
server.registerTool(
  ...
  async ({ semanticOnly }) => {
    ...
    const result = await getSemanticSurface(page, cdp, workerBridge, nodeIndex, { semanticOnly });
...
server.registerTool(
  'get_state_delta',
  {
    description:
      ...
      '• added — New nodes that appeared (with their roles, names, and IDs)\n' +
      '• removed — Nodes that disappeared\n' +
      '• modified — Nodes whose role, name, or children changed\n\n' +
...
server.registerTool(
  ...
  async ({ selector, visibleOnly, timeoutMs = 0 }) => {
    ...
    const searchSelector = isXPath ? `::-p-xpath(${selector.slice('xpath/'.length)})` : selector;
    ...
    do {
      for (const frame of page.frames()) {
        const handles = await frame.$$(isXPath ? searchSelector : selector);
        for (const handle of handles) {
          try {
            ...
            if (visibleOnly && !info.visible) continue;

            // Get backendNodeId via CDP
