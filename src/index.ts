...
DRIVE: atomic_interact({ action, backendNodeId | coordinate, ... }) — locate + act in one tick (click/type/hover/scroll/drag; has a waitFor param to act+wait in one call). browser_navigate({ url, waitUntil?, bypassCache? }).
